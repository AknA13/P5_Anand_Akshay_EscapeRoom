	...
	public boolean matchesItems(List<Item> items) {
		...
		if(orderMatters) {
			for(int i=0;i<items.size();i++) {
				if(!items.get(i).getName().equals(ingredients.get(i))) {
					return false;
				}
			}
			return true;
		}else {
			ArrayList<String> ingredientC=new ArrayList<String>();
			for(int i=0;i<ingredients.size();i++) {
				ingredientC.add(ingredients.get(i));
			}
			for(int i=0;i<items.size();i++) {
				boolean found=false;
				for(int j=0;j<ingredientC.size();j++) {
					if(items.get(i).getName().equals(ingredientC.get(j))) {
						found=true;
						ingredientC.remove(j);
						break;
					}
				}
				if(!found) {
